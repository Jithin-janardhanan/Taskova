...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      try {
        final response = await http.get(
          Uri.parse('https://anjalitechfifo.pythonanywhere.com/api/shopkeeper/profile/'),
          headers: {
            'Authorization': 'Bearer $_accessToken',
            'Content-Type': 'application/json',
          },
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            _name = personalProfile['name'] ?? prefs.getString('user_name') ?? 'User';
            _email = personalProfile['email'] ?? prefs.getString('user_email') ?? '';
            _phoneNumber = personalProfile['phone_number'] ?? '';
    ...
    try {
      ...
      if (_accessToken != null) {
        try {
          final response = await http
              .patch(
                Uri.parse('https://anjalitechfifo.pythonanywhere.com/api/shopkeeper/profile/update/'),
                headers: {
                  'Authorization': 'Bearer $_accessToken',
                  'Content-Type': 'application/json',
                },
    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        if (_accessToken != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('You need to be logged in to update your profile picture.'),
              backgroundColor: Colors.red,
            ),
          );
        }
            ...
            children: [
              CircleAvatar(
                ...
                backgroundImage: _profilePicture != null
                    ? CachedNetworkImageProvider('$_baseImageUrl$_profilePicture')
                    : null,
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                subtitle: Text(business['address'] ?? 'No address'),
                                onTap: () {
                                  setState(() {
                                    _selectedBusiness = business;
                                  });
